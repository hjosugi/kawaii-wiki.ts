/**
 * MySQL DB event bus contract test — integration.
 * Mirrors the SQLite DB-bus contract (cross-instance delivery without local
 * duplicates, resume-from-tail, pruning) against a real MySQL database, using two
 * clients on the same isolated database. Env-gated.
 */
import { afterAll, beforeAll, beforeEach, describe, expect, test } from 'bun:test'
import { asc } from 'drizzle-orm'
import { createMysqlContractDb, testMysqlUrl, withDatabase, type MysqlContractDb } from './test-support.ts'
import { createMysqlClient, type MysqlClient } from './client.ts'
import { createMysqlDbEventBus } from './event-bus.ts'
import { wikiEvents } from './schema.ts'
import type { WikiEvent } from '../../realtime/bus.ts'

const DATABASE = 'kw_mysql_eventbus'
...
describe.skipIf(!testMysqlUrl)('mysql db event bus', () => {
  let harness: MysqlContractDb
  let clientB: MysqlClient

  beforeAll(async () => {
    harness = await createMysqlContractDb(DATABASE)
    clientB = createMysqlClient({ driver: 'mysql', url: withDatabase(testMysqlUrl ?? '', DATABASE), ssl: false, maxConnections: 4 })
  }, 30_000)
  ...
  test('delivers events across instances without local duplicates', async () => {
    const busA = createMysqlDbEventBus(harness.client, { sourceId: 'a', pollIntervalMs: 10 })
    const busB = createMysqlDbEventBus(clientB, { sourceId: 'b', pollIntervalMs: 10 })
    try {
      const seenA: WikiEvent[] = []
      const seenB: WikiEvent[] = []
      busA.subscribe((event) => seenA.push(event))
      busB.subscribe((event) => seenB.push(event))
      await Bun.sleep(80) // let both buses initialize lastSeenId from the (empty) tail
  ...
  test('prunes stored events past the retention limit', async () => {
    const bus = createMysqlDbEventBus(harness.client, { sourceId: 'a', pollIntervalMs: 10, maxStoredEvents: 3 })
    try {
