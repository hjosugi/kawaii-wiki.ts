...
/** MySQL implementation of the driver-neutral page-read contract. */
export const createMysqlPageReadRepository = (db: MysqlDb): PageReadRepository => ({
  async listActive() {
...
/**
 * MySQL implementation of the driver-neutral page-write contract. Mutations
  ...
  async move(input) {
    ...
    const page = await db.transaction(async (tx) => {
      ...
      await tx.insert(pageRedirects)
        ...
        .onDuplicateKeyUpdate({ set: { toPath: input.newPath, createdAt: input.updatedAt } })
