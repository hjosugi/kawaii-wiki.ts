...
/** MySQL implementation of the driver-neutral admin contract. */
export const createMysqlAdminRepository = (db: MysqlDb): AdminRepository => ({
  async stats() {
  ...
  async listRevisionCandidates() {
    // SQLite tie-broke on rowid; MySQL has none, so use the id for a stable,
