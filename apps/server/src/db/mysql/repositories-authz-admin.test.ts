/**
 * MySQL authz / admin / page-read repository contract tests — integration.
 * Env-gated; own isolated database. Mirrors the Postgres contracts.
 */
...
import { createMysqlAuthzRepository } from './repositories/authz.ts'
import { createMysqlAdminRepository } from './repositories/admin.ts'
import { createMysqlPageReadRepository } from './repositories/pages.ts'
import { DuplicateAuthzGroupError, type PermissionGrantRecord } from '../../repositories/authz.ts'

describe.skipIf(!testMysqlUrl)('mysql authz/admin/page-read contracts', () => {
  let harness: MysqlContractDb
  // MySQL carries no DB default on the profile/body text columns, so fixtures supply them.
  const seedUser = (id: string, over: Record<string, unknown> = {}) =>
    harness.db.insert(users).values({
      id, email: `${id}@x`, name: id.toUpperCase(), passwordHash: 'h',
      profileBio: '', profileLinks: '[]', profileFavoritePages: '[]', createdAt: 1, ...over,
    })
  ...
  const seedPage = (id: string, path: string, over: Record<string, unknown> = {}) =>
    harness.db.insert(pages).values({
      id, path, title: path, content: '', renderedHtml: '', toc: '[]', labels: '[]', createdAt: 1, updatedAt: 1, ...over,
    })
  const seedRevision = (id: string, pageId: string, over: Record<string, unknown> = {}) =>
    harness.db.insert(pageRevisions).values({ id, pageId, path: 'docs/a', title: 'T', content: '', action: 'updated', createdAt: 1, ...over })

  beforeAll(async () => { harness = await createMysqlContractDb('kw_authz_contract') }, 30_000)
  ...
  test('authz: ensureDefaults is idempotent; grants and page rules', async () => {
    const repo = createMysqlAuthzRepository(harness.db)
  ...
  test('admin: stats and history stats', async () => {
    ...
    const repo = createMysqlAdminRepository(harness.db)
  ...
  test('page reads: active/inactive listings and redirects', async () => {
    ...
    const repo = createMysqlPageReadRepository(harness.db)
