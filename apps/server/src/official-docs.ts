...
export const OFFICIAL_DOCS_VERSION = '1.0.9'
...
const repositoryDocumentationSources: readonly RepositoryDocumentationSource[] = [
  ...
  { path: 'docs/reference/core-package', title: 'Coreパッケージ', description: '純粋なドメインロジックと型。', sourcePath: '../../../packages/core/README.md', locale: 'ja' },
]
...
const referencePage = (
  ...
  path: source.path,
  title: source.locale === 'ja' ? source.title : `${source.title}（英語原文）`,
  description: source.description,
  content: `> ソース: \`${source.sourcePath.replace(/^\.\.\/\.\.\/\.\.\//, '')}\`${source.locale === 'ja' ? '' : ' — このページはリポジトリの英語原文です。'}\n\n${readFileSync(new URL(source.sourcePath, import.meta.url), 'utf8')}`,
  labels: ['kawaii-wiki-docs'],
  status: 'verified',
...
利用者向けの公式ガイドは日本語で提供しています。このセクションはGitHubで管理している開発・運用Markdownの原文で、英語資料には「英語原文」と明記しています。通常は日本語ガイドを読み、厳密な設定値、設計判断、API互換性、RFCを確認するときだけ参照してください。

${repositoryDocumentationSources.map((source) => `- [[${source.path}|${source.title}${source.locale === 'ja' ? '' : '（英語原文）'}]] — ${source.description}`).join('\n')}`, 199),
