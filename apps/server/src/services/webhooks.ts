...
const defaultResolver: WebhookHostnameResolver = async (hostname) =>
...
const hostnameForValidation = (url: URL): string => url.hostname.replace(/^\[|\]$/g, '')

const normalizeWebhookUrl = (value: string): Result<URL, AppError> => {
  try {
...
const ensurePublicLiteralTarget = (url: URL): Result<true, AppError> => {
...
const cleanTargetUrl = (value: string, allowPrivateTargets: boolean): Result<string, AppError> => {
