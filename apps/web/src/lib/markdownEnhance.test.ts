...
describe('markdown enhancements', () => {
  test('enhances code blocks and content tabs without duplicating controls', () => {
    const renderer = rendererForMarkdownFeatures({ enableMath: false, enableEmoji: true, enableMermaid: false })
...
\`\`\``).html}`
    ...
    enhanceRenderedMarkdown(root, { enableMath: false, enableEmoji: true, enableMermaid: false })
