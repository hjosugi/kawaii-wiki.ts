...
> **Status: v0.4.10** — a small, complete, runnable wiki: Markdown pages with visual editing,
...
> runtime branding, configurable site locale/date defaults, and a typed API.
...
```bash
docker pull ghcr.io/hjosugi/ts-wiki:v0.4.10
docker volume create ts-wiki-data
export JWT_SECRET="$(openssl rand -hex 32)"
docker run --rm -v ts-wiki-data:/data \
  -e JWT_SECRET="$JWT_SECRET" \
  -e TS_WIKI_SEED_ADMIN_PASSWORD="change-me-before-first-seed" \
  ghcr.io/hjosugi/ts-wiki:v0.4.10 bun --filter '@ts-wiki/server' db:seed
docker run -d --name ts-wiki --restart unless-stopped \
  -p 4000:4000 -v ts-wiki-data:/data \
  -e NODE_ENV=production \
  -e JWT_SECRET="$JWT_SECRET" \
  ghcr.io/hjosugi/ts-wiki:v0.4.10
```
